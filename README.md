...
- `plots/` (generated by plotting script)
...
## Plot Metrics and Original-vs-Predicted

Generate all figures from a run directory:

```bash
PYTHONPATH=src python3 scripts/plot_metrics_and_comparison.py --run-dir outputs/latest
```

Include GPU predictions in the same plots:

```bash
PYTHONPATH=src python3 scripts/plot_metrics_and_comparison.py \
  --run-dir outputs/latest \
  --gpu-predictions outputs/latest/gpu/predictions_gpu_xgboost.csv
```

Generated plots (under `outputs/latest/plots` by default):

- `lead_time_metrics.png`
- `nowcast_roc_curve.png`
- `nowcast_pr_curve.png`
...
## One-Command Run + Plot

Run pipeline, optional GPU model, and all plots:
...
```bash
RUN_GPU=0 bash scripts/run_all_with_plots.sh
```
